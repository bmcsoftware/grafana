...
func addOrgMigrations(mg *Migrator) {
	orgV1 := Table{
		...
		//     },
		// End kavraham, remove unique index to allow different tenant (org) with the same name (https://jira.bmc.com/browse/DRJ71-730)
	}
	...
	// Start kavraham, remove unique index from `name` coloumn in `org` table.
	mg.AddMigration("drop 'UQE_org_name' index ", NewDropIndexMigration(orgV1, &Index{Cols: []string{"name"}, Type: UniqueIndex}))
	// End kavraham, remove unique index from `name` coloumn in `org` table if exist.
