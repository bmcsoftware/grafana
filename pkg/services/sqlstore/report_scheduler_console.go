...
func init() {
	bus.AddHandler("sql", GetRSJobQueueByJobId)
	bus.AddHandler("sql", GetReportListJobQueue)
	bus.AddHandler("sql", GetJobQueuesByReportId)
}

func GetReportListJobQueue(ctx context.Context, query *models.GetReportListJobQueue) error {
	results := make([]*models.RSReportInfo, 0)
	rawSql := fmt.Sprintf(`SELECT r.id, map.job_id, r.name, r.report_type, r.enabled, d.title, d.uid,
	r.created_at, r.updated_at, r.next_at, r.last_at, u.name as created_by, u.id as user_id,
	t.count_runs, f.count_fail, s.description as last_fail,
	...
	AND r.org_id = %d`, "%"+query.Query+"%", query.OrgID)
	err := x.SQL(rawSql).
		Find(&results)
	if err != nil {
		return err
...
func GetRSJobQueueByJobId(ctx context.Context, query *models.GetRSJobQueueByJobId) error {
	queue := &models.RSJobQueue{}

	_, err := x.Table("job_queue").
...
func GetJobQueuesByReportId(ctx context.Context, query *models.GetRSJobQueues) error {
	queues := make([]*models.RSJobQueue, 0)

	sqlQuery := x.Table("job_queue").
		Join("RIGHT", "report_data", "report_data.id = job_queue.report_data_id").
		Where("report_data.org_id = ?", query.OrgID).
		Where("job_queue.report_data_id = ?", query.ReportId).
		OrderBy("job_queue.id desc").
		Limit(query.Limit, 0)

	if !query.IsOrgAdmin {
		sqlQuery = sqlQuery.Where("report_data.user_id = ?", query.UserID)
	}

	err := sqlQuery.Find(&queues)
	if err != nil {
		return err
	}

	result := make([]*models.GetRSJobQueue, 0)
