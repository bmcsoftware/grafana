...
func (ss *SQLStore) DeleteUserFromRS(ctx context.Context, query *models.DeleteUserFromRS) error {
	return ss.WithTransactionalDbSession(ctx, func(sess *DBSession) error {
		...
		//-- After recipient removal, when report is created by the user in question, disable it.
		disableSQL := fmt.Sprintf(`UPDATE report_data SET enabled = false
		WHERE ((recipients IS NULL OR recipients = '') AND schedule_type = 'email') OR user_id=%d;`, query.UserId)
...
func updateRSData(sess *DBSession, query *models.UpdateRS) error {
	...
	if _, err := sess.Table("report_data").
		...
		Nullable("filter", "time_range", "csv_delimiter").
		UseBool("has_date_stamp", "has_time_stamp", "compress_attachment", "table_scaling").
