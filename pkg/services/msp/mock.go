...
func MockMspCtx(ctx *models.ReqContext) {
	...
	ctx.IsOrg0User = util.Contains(mspListMock, "0")
	ctx.HasExternalOrg = true

	printMockCtxMspLog(ctx)
}

func printMockCtxMspLog(ctx *models.ReqContext) {
	...
	if len(ctx.MspOrgs) == 1 && util.Contains(ctx.MspOrgs, "0") {
		ctx.Logger.Info("User is associated only with organization zero",
			...
			"IsOrg0User", ctx.IsOrg0User, "Orgs", strings.Join(ctx.MspOrgs, ","),
		)
		return
	}
