...
func (s *UserSync) BHDRoleUpdate(ctx context.Context, id *authn.Identity, r *authn.Request) error {
	...
	if external.FeatureFlagBhdRBAC.Enabled(r.HTTPRequest, signedInUser) && setting.Env != setting.Dev {
		if (r.DecodedToken != nil && bmc.ContainsLower(r.DecodedToken.Permissions, string('*'))) || *id.IsGrafanaAdmin {
			...
			if err != nil || len(roles) == 0 {
				s.log.Warn("Failed to get bhd roles", "error", err)
				err = fallbacktoJwt(ctx, id, r.DecodedToken)
				if err != nil {
					s.log.Error("Dashboard permissions are missing", "error", err)
					return err
				}
			} else {
				id.BHDRoles = roles
		...
		// if r.DecodedToken != nil && reqContext.IsSignedIn {
		if r.DecodedToken != nil {
			updateRole(ctx, id, r.DecodedToken)
		}
	}
	return nil
}
...
func fallbacktoJwt(ctx context.Context, id *authn.Identity, jwtTokenDetails *authz.UserInfo) error {
	...
	if bmc.ContainsLower(jwtTokenDetails.Permissions, bmc.ReportingAdmin) {
		roles = append(roles, 1)
		id.BHDRoles = roles
		id.OrgRoles[id.OrgID] = org.RoleAdmin
	} else if bmc.ContainsLower(jwtTokenDetails.Permissions, bmc.ReportingEditor) {
		roles = append(roles, 2)
		id.BHDRoles = roles
		id.OrgRoles[id.OrgID] = org.RoleEditor
	} else if bmc.ContainsLower(jwtTokenDetails.Permissions, bmc.ReportingViewer) {
...
func (s *UserSync) CreateUnrestrictedTeam(ctx context.Context, id *authn.Identity) {
	...
	s.teamService.CreateTeam("Unrestricted Access", "", id.OrgID, mspUnrestrictedTeamID, true)
