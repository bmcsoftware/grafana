...
func (h *ContextHandler) Middleware(mContext *web.Context) {
	...
	if encodedJWTToken != "" {
		...
		reqContext.Logger.Debug("usrObj-UserID ", "UserID", usrObj.UserID)
		reqContext.Logger.Debug("usrObj-Tenant_Id ", "Tenant_Id", usrObj.Tenant_Id)
		reqContext.Logger.Debug("usrObj-Groups ", "Groups", len(usrObj.Groups))
...
func updateTeamMembership(ctx *models.ReqContext, jwtTokenDetails *authz.UserInfo) {
	if jwtTokenDetails != nil {
		...
		for i := 0; i < len(jwtTokenDetails.Groups); i++ {
			...
			if err := bus.DispatchCtx(ctx.Req.Context(), &query); err == nil && len(query.Result) > 0 {
				...
				if err := sqlstore.AddTeamMember(&cmd); err != nil {
...
func removeFromTeam(ctx *models.ReqContext, jwtTokenDetails *authz.UserInfo) {
	if jwtTokenDetails != nil {
		...
		if err := bus.DispatchCtx(ctx.Req.Context(), &getTeamsByUserQuery); err != nil {
			ctx.Logger.Debug("Team not found")
			return
		}
		...
		for _, t := range getTeamsByUserQuery.Result {
			...
			if !contains(jwtTokenDetails.Groups, strconv.FormatInt(int64(teamId), 10)) {
				if err := bus.DispatchCtx(ctx.Req.Context(), &models.RemoveTeamMemberCommand{OrgId: orgId, TeamId: teamId, UserId: userId}); err != nil {
					if err == models.ErrTeamNotFound {
						ctx.Logger.Debug("Team not found")
					}
...
func CheckIfUserSynced(rCtx *models.ReqContext) {
	logger := log.New("auth.proxy")
	// Remove user if ID is not synced with ade-ID
	encodedJWTToken := rCtx.Req.Header.Get("X-JWT-Token")
	if encodedJWTToken != "" {
		logger.Info("Checking if user is synced with ade")
		decodedToken, err := authz.Authorize(encodedJWTToken)
		if err != nil {
			logger.Error("Failed to decode JWT Token", "err", err.Error())
		}
		ImsUserID, _ := strconv.ParseInt(decodedToken.UserID, 10, 64)
		if decodedToken != nil && ImsUserID != rCtx.UserId {
			logger.New("user").Info("User not synced - removing user")
			removeUnsyncedUser := &models.DeleteUserCommand{
				UserId: rCtx.UserId,
			}
			if err := bus.DispatchCtx(rCtx.Req.Context(), removeUnsyncedUser); err != nil {
				logger.Info("BMC Custom - Unable to remove unsynced user", "err", err.Error())
			}
		}
	}
}
