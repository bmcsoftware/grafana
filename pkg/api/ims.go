...
func GetImsUserInfo(c *models.ReqContext) response.Response {
	...
	jwtToken, err := GetServiceAccountToken(c.OrgId)
	if err != nil {
		imsLog.Error(err.Error())
		return response.Error(http.StatusInternalServerError, "Internal server error", err)
	}
	impJwtToken, err := GetServiceImpersonationToken(c.UserId, jwtToken)
	if err != nil {
		imsLog.Error(err.Error())
		return response.Error(http.StatusInternalServerError, "Internal server error", err)
	}
	//Fetch user info with preferences
	url := imsServiceURL + "/ims/api/v1/userinfo?preferences=true&gs_product_id=dashboards"
	method := "GET"
	...
	req.Header.Add("Authorization", "Bearer "+impJwtToken)
