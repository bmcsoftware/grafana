...
func (p *PluginsAPI) SearchTeam(c *contextmodel.ReqContext) response.Response {
	...
	logger.Log.Info("Teams search request", "r", request)
	result, err := bhd_team.SearchTeam(c.Req.Context(), p.store, &request)
	if err != nil {
		logger.Log.Error("Failed to search Teams", "error", err)
		return response.Error(500, "Failed to search Teams", err)
	}
	result.Page = page
	result.PerPage = perPage
	logger.Log.Info("Team search request processed successfully", "Total Count", result.TotalCount)
	return response.JSON(http.StatusOK, &result)
}

func (p *PluginsAPI) AddTeamBHDRole(c *contextmodel.ReqContext) response.Response {
	logger.Log.Info("Request to assign Dashboard Role to the Team")
	...
	logger.Log.Info("Dashboard Role assigned successfully to the Team")
	return response.Success("Team role added.")
}

func (p *PluginsAPI) RemoveTeamBHDRole(c *contextmodel.ReqContext) response.Response {
	logger.Log.Info("Request to remove Dashboard Role assigned to the Team")
	cmd := bhd_team.AddTeamRoleCommand{}
	var err error
	cmd.OrgID = c.OrgID
	cmd.ID, err = strconv.ParseInt(web.Params(c.Req)[":teamId"], 10, 64)
	...
	logger.Log.Info("Dashboard Role assigned to the Team, removed successfully")
