...
func (p *PluginsAPI) getAllDashboards(c *contextmodel.ReqContext) response.Response {
	...
	tags, err := url.QueryUnescape(c.Query("tags"))
	if err != nil {
		model.Log.Error("Error decoding query param Tags", "error")
	}
	folder_name, err := url.QueryUnescape(c.Query("folder_name"))
	if err != nil {
		model.Log.Error("Error decoding query param Folder Name", "error")
	}
