...
func (p *PluginsAPI) extractPanels(panelJson interface{}, dsList *[]string, inputs map[string]*simplejson.Json) *simplejson.Json {
	...
	if checkGet {
		...
		if dsUid != "" && dsType != "-- Mixed --" && dsType != "grafana" && dsType != "-- Dashboard --" {
			*dsList = append(*dsList, dsUid)
			var updatedUid string
			if val, ok := inputs[dsUid]; ok {
				updatedUid = val.Get("name").MustString("")
				if updatedUid != "" {
					panel.SetPath([]string{"datasource", "type"}, fmt.Sprintf("${%s}", updatedUid))
				}
			} else {
				updatedUid = strings.ToUpper(dsType)
				if updatedUid != "" {
					panel.SetPath([]string{"datasource", "type"}, fmt.Sprintf(updatedUid))
				}
			}
		}
		...
		for _, target := range targets {
			...
			if targetJson != nil {
				dsUid := targetJson.GetPath("datasource", "uid").MustString("")
				dsType := targetJson.GetPath("datasource", "uid").MustString("")
				if dsUid != "" && dsType != "-- Mixed --" && dsType != "grafana" && dsType != "-- Dashboard --" {
					*dsList = append(*dsList, dsUid)
					var updatedUid string
					if val, ok := inputs[dsUid]; ok {
						updatedUid = val.Get("name").MustString("")
						if updatedUid != "" {
							targetJson.SetPath([]string{"datasource", "type"}, fmt.Sprintf("${%s}", updatedUid))
						}
					} else {
						updatedUid = strings.ToUpper(dsType)
						if updatedUid != "" {
							targetJson.SetPath([]string{"datasource", "type"}, fmt.Sprintf(updatedUid))
						}
					}
