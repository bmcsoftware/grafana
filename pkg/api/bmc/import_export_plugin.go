...
func (p *PluginsAPI) ImportPlugin(c *models.ReqContext) response.Response {
	return response.Success("Not implemented yet")
}

func (p *PluginsAPI) ExportPlugin(c *models.ReqContext) response.Response {
	...
	for _, dash := range dashboards {
		...
		var libList map[string]libraryelements.LibraryElementDTO
		var err error
		...
		dsList := p.prepareDatasource(c.Req.Context(), dash, datasources, libList)
		varList := p.prepareVariables(c.Req.Context(), dash)
...
func (p *PluginsAPI) GetDashboards(ctx context.Context, orgId int64, query *plugin.GetDashQuery) error {
	return p.store.WithDbSession(ctx, func(sess *sqlstore.DBSession) error {
		...
		// Get folders by uids
		folders := make([]*models.Dashboard, 0)
		if err := sess.
		...
		dashboards1 := make([]*models.Dashboard, 0)
		if err := sess.
		...
		dashboards2 := make([]*models.Dashboard, 0)
		if err := sess.
...
func (p *PluginsAPI) prepareLibraryPanels(c context.Context, dash *models.Dashboard) (map[string]libraryelements.LibraryElementDTO, error) {
...
func (p *PluginsAPI) prepareDatasource(c context.Context, dashboard *models.Dashboard, inputs map[string]*simplejson.Json, libList map[string]libraryelements.LibraryElementDTO) []*simplejson.Json {
	...
	// remove duplicates
	dsList = util.RemoveDuplicates(dsList)

	dashboardDatasources := make([]*simplejson.Json, 0)
	for _, dsUID := range dsList {
...
func (p *PluginsAPI) prepareVariables(c context.Context, dashboard *models.Dashboard) []*simplejson.Json {
...
func (p *PluginsAPI) prepareJsonFiles(dashboards []*models.Dashboard, folderDest string) *plugin.ErrorResult {
	...
	for _, dashboard := range dashboards {
		...
		fileName := models.SlugifyTitle(dashboard.Title)
