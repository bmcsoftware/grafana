...
func (hs *HTTPServer) SetReportBrandingSettings(c *contextmodel.ReqContext) response.Response {
	...
	if valid := util.ValidateUrlScheme(cmd.LogoUrl); cmd.LogoUrl != "" && !valid {
		return response.Error(http.StatusBadRequest, "Invalid logo URL", err)
	}

	if valid := util.ValidateUrlScheme(cmd.FooterTextUrl); cmd.FooterTextUrl != "" && !valid {
		return response.Error(http.StatusBadRequest, "Invalid footer URL", err)
	}
