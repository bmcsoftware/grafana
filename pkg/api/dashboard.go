...
func isRbacSqlEnabled(c *contextmodel.ReqContext, hs *HTTPServer) bool {
	...
	isOrgAdmin := orgRole == roletype.RoleAdmin
	...
	isSqlEnabledinRbacValue := isSqlEnabledinRbac(c, hs)
	isValidAccessValue := isValidAccess(isOrgAdmin, isAppliedToAdmins)

	hs.log.Debug(fmt.Sprintf("TenantId: %d - Condition values: isSqlEnabledinDefaultPreferences=%v, isSqlEnabledinRbac=%v, isValidAccess=%v, isAppliedToAdmins=%v",
		c.SignedInUser.GetOrgID(), isSqlEnabledinDefaultPreferences, isSqlEnabledinRbacValue, isValidAccessValue, isAppliedToAdmins))

	if !isSqlEnabledinDefaultPreferences && !isSqlEnabledinRbacValue && isValidAccessValue {
		hs.log.Debug(fmt.Sprintf("TenantId: %d - SQL is not enabled in preferences and no SQL permission in RBAC.", c.SignedInUser.GetOrgID()))
		return false
	}

	hs.log.Debug(fmt.Sprintf("TenantId: %d - SQL is either enabled in preferences or SQL is granted through RBAC.", c.SignedInUser.GetOrgID()))
	return true
}
...
func getPreferences(c *contextmodel.ReqContext, hs *HTTPServer) (bool, bool) {
	...
	enabledTypes, ok := preferencesData["enabledQueryTypes"].(map[string]interface{})["enabledTypes"].([]interface{})
	if !ok {
		hs.log.Debug(fmt.Sprintf("TenantId: %d - Unable to retrieve enabledTypes in enabledQueryTypes", c.SignedInUser.GetOrgID()))
		return false, false
	}
	...
	if applyForAdmin, ok := preferencesData["enabledQueryTypes"].(map[string]interface{})["applyForAdmin"].(bool); ok {
...
func isValidAccess(isOrgAdmin, isAppliedToAdmins bool) bool {
	return isOrgAdmin && isAppliedToAdmins || !isOrgAdmin
}
