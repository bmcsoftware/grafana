...
func DeleteCalcFieldsById(c *models.ReqContext) response.Response {
	...
	if err1 := sqlstore.GetDashboardsToCalcDelete(&cmd); err1 != nil {
...
func ModifyCalcFieldsById(c *models.ReqContext) response.Response {
	...
	cmd.OrgId = c.OrgId
	if err := sqlstore.ModifyCalcFields(c.Req.Context(), &cmd); err != nil {
		return response.Error(500, "Failed to modify calculated field", err)
	}
	if err1 := sqlstore.GetDashboardsToCalcUpdate(cmd.Name, cmd.OrgId, cmd.Module, cmd.SqlQuery, cmd.Name); err1 != nil {
