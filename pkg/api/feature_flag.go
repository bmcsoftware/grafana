...
func GetTenantFeaturesFromService(tenantId int64) []Tenantfeatures {
	...
	if !setting.FeatureFlagEnabled {
		...
		logger.Info("GetTenantFeaturesFromService URL:" + setting.FeatureFlagEndpoint + "/tenantfeatures?Tenant=" + strconv.Itoa(int(tenantId)))

		client := http.Client{}
		req, _ := http.NewRequest("GET", setting.FeatureFlagEndpoint+"/tenantfeatures?Tenant="+strconv.Itoa(int(tenantId)), nil)
