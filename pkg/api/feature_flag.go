...
func GetTenantFeaturesFromService(tenantId int64) []Tenantfeatures {
	if !setting.FeatureFlagEnabled {
		...
		log.Infof("GetTenantFeaturesFromService URL:" + setting.FeatureFlagEndpoint + "/tenantfeatures?Solution=dashboards&Tenant=" + strconv.Itoa(int(tenantId)))
		...
		if err != nil {
			log.Errorf(500, "Failed to get token")
			var emptyResponse TenantFeatureResponse
			emptyResponse.Tenantfeatures = make([]Tenantfeatures, 0)
			return emptyResponse.Tenantfeatures
		}
		...
		if res != nil {
			log.Infof("GetTenantFeaturesFromService status:" + res.Status)
			if res.StatusCode != 200 {
				log.Infof(res.Status)
				log.Infof("status is not 200 returning empty array")
				var emptyResponse TenantFeatureResponse
				emptyResponse.Tenantfeatures = make([]Tenantfeatures, 0)
				return emptyResponse.Tenantfeatures
			}
		} else {
			log.Infof("result set is null or tenant feature flag service is not available, returning empty array")
			var emptyResponse TenantFeatureResponse
			emptyResponse.Tenantfeatures = make([]Tenantfeatures, 0)
			return emptyResponse.Tenantfeatures
		}
