...
func (hs *HTTPServer) registerSchedulerRoutes() {
	...
	r.Group("/api", func(apiRoute routing.RouteRegister) {
		...
		// API's for storing ftp configuration (org admin permission required)
		apiRoute.Group("/reports/config", func(schedulerRoute routing.RouteRegister) {
			schedulerRoute.Get("/ftp", routing.Wrap(hs.GetFTPConfig))
			schedulerRoute.Post("/ftp", routing.Wrap(hs.SetFTPConfig))
			schedulerRoute.Put("/ftp", routing.Wrap(hs.ModifyFTPConfig))
		}, reqOrgAdmin)
		...
		apiRoute.Group("/reports", func(schedulerRoute routing.RouteRegister) {
			...
			schedulerRoute.Get("/", routing.Wrap(hs.GetRS))
			schedulerRoute.Get("/:id", routing.Wrap(hs.GetRSById))
			schedulerRoute.Post("/dashboard", routing.Wrap(hs.GetRSByDashIds))

			schedulerRoute.Post("/mail", routing.Wrap(hs.ReportSendMail))
			schedulerRoute.Post("/execute", routing.Wrap(hs.ReportExecuteOnce))
			schedulerRoute.Get("/tenant/users", routing.Wrap(hs.GetTenantUsers))

			schedulerRoute.Post("/create", routing.Wrap(hs.InsertRS))
			schedulerRoute.Put("/update", routing.Wrap(hs.UpdateRS))
			schedulerRoute.Post("/delete", routing.Wrap(hs.DeleteRS))

			schedulerRoute.Post("/delete/dashboard", routing.Wrap(hs.DeleteRSByDashIds))
			schedulerRoute.Post("/enable", routing.Wrap(hs.EnableRS))
			schedulerRoute.Post("/disable", routing.Wrap(hs.DisableRS))
		}, reqEditorRole)

		apiRoute.Group("/reports/settings/branding", func(branding routing.RouteRegister) {
			branding.Get("/", routing.Wrap(hs.GetReportBrandingSettings), reqEditorRole)
			branding.Post("/", routing.Wrap(hs.SetReportBrandingSettings), reqOrgAdmin)
			branding.Delete("/", routing.Wrap(hs.DeleteReportBrandingSettings), reqOrgAdmin)
		})
