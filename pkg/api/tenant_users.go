...
func (hs *HTTPServer) getTenantUsers(c *models.ReqContext) []*org.OrgUserDTO {
	query := c.Query("query")
	...
	if !c.SignedInUser.HasExternalOrg || c.IsOrg0User {
		imsUserListResult := getTenantUsersHelper(c.Req.Context(), imsQuery, c.SignedInUser, "")
...
func getTenantUsersHelper(ctx context.Context, data *org.GetOrgUsersQuery, user *user.SignedInUser, mspOrgID string) []*org.OrgUserDTO {
	...
	if user.HasExternalOrg {
		queryParams.Add("orgId", mspOrgID)
	}
