...
func (hs *HTTPServer) GetRS(c *models.ReqContext) response.Response {
	query := &models.GetAll{
		UserId:      c.UserId,
		OrgId:       c.OrgId,
	...
	data, err := hs.SelectAll(c.Req.Context(), query)
	if err != nil {
...
func (hs *HTTPServer) GetRSById(c *models.ReqContext) response.Response {
	...
	data, err := hs.SelectById(c.Req.Context(), query)
	if err != nil {
		return hs.FailResponse(err)
	}
	return hs.SuccessResponse(reportToJson(data))
}
func (hs *HTTPServer) GetRSByDashIds(c *models.ReqContext) response.Response {
	cmd := dtos.ListRS{}
	...
	data, err := hs.SelectByDashIds(c.Req.Context(), query)
	if err != nil {
		return hs.FailResponse(err)
	}
	result := make([]*dtos.RSData, 0)
...
func (hs *HTTPServer) InsertRS(c *models.ReqContext) response.Response {
	cmd := dtos.InsertRS{}
	if err := web.Bind(c.Req, &cmd); err != nil {
		return response.Error(http.StatusBadRequest, "bad request data", err)
	}
	if err := hs.checkReportLimitReached(c.Req.Context(), c.OrgId); err != nil {
		return hs.FailResponse(err)
	}

	if cmd.ScheduleType != "ftp" {
		// Check restrictions on domains
		domains, isRestricting := hs.getEmailDomainRestrictions(c)
		if isRestricting {
			cmd.Recipients = util.EmailDomainValidator(cmd.Recipients, domains)
			if len(cmd.Recipients) == 0 {
				return hs.FailResponse(fmt.Errorf("No valid email set, email domains are all restricted"))
			}
		}
	}
	...
	query.Data.ScheduleType = cmd.ScheduleType
	query.Data.ServerDir = cmd.ServerDir

	if err := hs.SQLStore.InsertRS(c.Req.Context(), query); err != nil {
		return hs.FailResponse(err)
	}

	return hs.SuccessResponse(CustomResponse{Id: query.Data.Id, Message: Created})
}
func (hs *HTTPServer) UpdateRS(c *models.ReqContext) response.Response {
	cmd := dtos.UpdateRS{}
	if err := web.Bind(c.Req, &cmd); err != nil {
		return response.Error(http.StatusBadRequest, "bad request data", err)
	}
	...
	report, err := jsonToUpdateReport(&cmd, c.UserId)
	if err != nil {
		return hs.FailResponse(err)
	}
	...
	query.Data.OrgId = c.OrgId
	query.Data.UpdatedAt = time.Now().UTC()
	query.IsOrgAdmin = c.OrgRole == models.ROLE_ADMIN

	if err := hs.SQLStore.UpdateRS(c.Req.Context(), query); err != nil {
		return hs.FailResponse(err)
	}
	logger.Info("Report Update", "Report_ID", query.Data.Id, "Report_Name", query.Data.Name, "Org_ID", c.OrgId, "Updated_by", c.UserId)
	return hs.SuccessResponse(CustomResponse{Id: query.Data.Id, Message: Updated})
}
func (hs *HTTPServer) DeleteRS(c *models.ReqContext) response.Response {
	cmd := dtos.ListRS{}
	if err := web.Bind(c.Req, &cmd); err != nil {
		return response.Error(http.StatusBadRequest, "bad request data", err)
	}
	query := models.DeleteRS{Ids: cmd.Ids, UserId: c.UserId, OrgId: c.OrgId, IsOrgAdmin: c.OrgRole == models.ROLE_ADMIN}
	if err := hs.SQLStore.DeleteRS(c.Req.Context(), &query); err != nil {
		return hs.FailResponse(err)
	}
...
func (hs *HTTPServer) EnableRS(c *models.ReqContext) response.Response {
	...
	query := models.EnableRS{Ids: cmd.Ids, UserId: c.UserId, OrgId: c.OrgId, IsOrgAdmin: c.OrgRole == models.ROLE_ADMIN}
...
func (hs *HTTPServer) DisableRS(c *models.ReqContext) response.Response {
	...
	query := models.DisableRS{Ids: cmd.Ids, UserId: c.UserId, OrgId: c.OrgId, IsOrgAdmin: c.OrgRole == models.ROLE_ADMIN}
...
func (hs *HTTPServer) ReportExecuteOnce(c *models.ReqContext) response.Response {
	...
	query := &models.GetJobById{
		...
		UserId:     c.UserId,
		OrgId:      c.OrgId,
		IsOrgAdmin: c.OrgRole == models.ROLE_ADMIN,
	}
...
func (hs *HTTPServer) GetTenantUsers(c *models.ReqContext) response.Response {
	result := hs.getTenantUsers(c.Req.Context(), c.OrgId, c.Query("query"), c.QueryInt("limit"))
...
func jsonToInsertReport(json *dtos.InsertRS) (*models.InsertRS, error) {
	...
	if json.ScheduleType != "ftp" {
		if len(json.Recipients) == 0 {
			return nil, fmt.Errorf("No valid recipients set")
		}
	}
	...
	report := &models.InsertRS{
		Data: models.InsertRSData{
			...
			ServerDir:    json.ServerDir,
			HasDateStamp: json.HasDateStamp,
...
func jsonToUpdateReport(json *dtos.UpdateRS, id int64) (*models.UpdateRS, error) {
	// Sanitize fields
	json.Name = util.SanitizeHtml(json.Name)
	json.Description = util.SanitizeHtml(json.Description)
	json.Subject = util.SanitizeHtml(json.Subject)
