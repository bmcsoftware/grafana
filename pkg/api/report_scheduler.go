...
func InsertRS(c *models.ReqContext) response.Response {
	...
	domains, isRestricting := getEmailDomainRestrictions(c)
	if isRestricting {
		cmd.Recipients = util.EmailDomainValidator(cmd.Recipients, domains)
		if len(cmd.Recipients) == 0 {
			return FailResponse(fmt.Errorf("No valid email set, email domains are all restricted"))
		}
	}
...
func RemoveOrDisableOrgSchedules(c *models.ReqContext) response.Response {
	orgId, err := util.ParamsInt64(c.Req, ":orgId")
	if err != nil {
		return FailResponse(models.ErrInvalidId)
	}
...
func getEmailDomainRestrictions(c *models.ReqContext) ([]string, bool) {
	settings, err := getReportSchedulerSettings(c)
	if err != nil {
		return []string{}, false
	}
