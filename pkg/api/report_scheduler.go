...
func (hs *HTTPServer) ReportPDFPreview(c *contextmodel.ReqContext) response.Response {
	...
	if cmd.ReportType == "html" {
		enabled := external.FeatureFlagReportingTypeHTML.Enabled(c.Req, c.SignedInUser)
		if !enabled {
			return response.Error(http.StatusBadRequest, "Feature not enabled", nil)
		}
	}
...
func (hs *HTTPServer) ReportSendMail(c *contextmodel.ReqContext) response.Response {
	...
	cmd.Sender = util.SanitizeHtml(cmd.Sender)
