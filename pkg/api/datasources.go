...
func (hs *HTTPServer) DeleteDataSourceByUID(c *models.ReqContext) response.Response {
	...
	data := kafkaproducer.Data{
		...
		ActivityTime:     kafkaproducer.EventTime(ds.Updated),
...
func (hs *HTTPServer) AddDataSource(c *models.ReqContext) response.Response {
	...
	if err := hs.DataSourcesService.AddDataSource(c.Req.Context(), &cmd); err != nil {
		...
		data := kafkaproducer.Data{
			...
			TenantID:         strconv.FormatInt(c.OrgID, 10),
			ObjectCategory:   "DATASOURCE",
			ObjectName:       cmd.Result.Name,
			ObjectType:       cmd.Result.Type,
			ObjectDetails:    "Datasource of type: " + cmd.Result.Type,
			Operation:        c.Context.Req.Method,
			OperationSubType: "Failed to add datasource",
			OperationType:    "ADD_DATASOURCE",
			OperationStatus:  "FAILED",
			ActorUserID:      strconv.FormatInt(c.UserID, 10),
			ActorLoginID:     loginName,
			ActivityTime:     kafkaproducer.EventTime(cmd.Result.Created),
			Source:           kafkaproducer.LookUpIp(c.Req.Header.Get("Origin")),
		}
		instance := kafkaproducer.GetInstance()
		instance.SendKafkaEvent(data)
...
func (hs *HTTPServer) UpdateDataSourceByID(c *models.ReqContext) response.Response {
	...
	if resp := validateSSRF(cmd.Type, cmd.Url, platformURL); resp != nil {
		return resp
	}
	// End
...
func (hs *HTTPServer) updateDataSourceByID(c *models.ReqContext, ds *datasources.DataSource, cmd datasources.UpdateDataSourceCommand) response.Response {
	...
	if err != nil {
		...
		data := kafkaproducer.Data{
			...
			Operation:        c.Context.Req.Method,
			OperationSubType: "Failed to update datasource",
	...
	data := kafkaproducer.Data{
		...
		ActorUserID:      strconv.FormatInt(c.UserID, 10),
		ActorLoginID:     loginName,
		ActivityTime:     kafkaproducer.EventTime(query.Result.Updated),
		Source:           kafkaproducer.LookUpIp(c.Req.Header.Get("Origin")),
	}
	instance := kafkaproducer.GetInstance()
	instance.SendKafkaEvent(data)
...
// BMC code
// author ateli - Mitigation for SSRF issue - DRJ71-3206
func validateSSRF(cmdType string, url string, platformURL string) response.Response {
	if _, err := datasource.ValidateSSRF(cmdType, url, platformURL); err != nil {
