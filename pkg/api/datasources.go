...
func (hs *HTTPServer) UpdateDataSourceByID(c *models.ReqContext) response.Response {
	...
	if resp := validateSSRF(cmd.Type, cmd.Url, platformURL, rmsMetadataURL); resp != nil {
		return resp
	}
	// End
...
// BMC code
// author ateli - Mitigation for SSRF issue - DRJ71-3206
func validateSSRF(cmdType string, url string, platformURL string, rmsMetadataURL string) response.Response {
	if _, err := datasource.ValidateSSRF(cmdType, url, platformURL, rmsMetadataURL); err != nil {
