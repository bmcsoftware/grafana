...
func (hs *HTTPServer) Usagedata(c *contextmodel.ReqContext) response.Response {
	...
	if strings.EqualFold(predefinedQuery, "plugininfo") {
		...
		res, err := hs.usagedataService.GetUserCount(c.Req.Context(), c.OrgID)
		if err != nil {
			return hs.FailResponse(err)
		}
		return hs.SuccessResponse(res)
		...
		if strings.EqualFold(predefinedQuery, "schedule") {
			...
			return hs.SuccessResponse(res)
		} /* else if strings.EqualFold(predefinedQuery, "hitcount") {

			res, err := hs.usagedataService.GetDashboardHitCount(c.Req.Context(), c.OrgID, c.OrgName, startTime, endTime)
			if err != nil {
				return hs.FailResponse(err)
			}
