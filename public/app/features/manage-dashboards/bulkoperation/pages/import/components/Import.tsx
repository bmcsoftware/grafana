...
import { Button, useStyles2, useTheme2, Modal, Icon, Label, Spinner, Legend } from '@grafana/ui';
import { notifyApp } from 'app/core/actions';
import { createErrorNotification } from 'app/core/copy/appNotification';

import { testIds } from '../../testIds';
import { useImportOperations, initialImportStatus } from '../state/actions';

import { DashboardsOverview } from './DashboardsOverview';

export const bulkLimit = (config.bootData.settings as any).bulkLimit ?? 10;
export const bulkLimitMsg = `Select upto ${bulkLimit} dashboards only`;
  ...
  const importAll = React.useMemo(() => {
    return () => {
      ...
      importOperations
        ...
        .catch((err: Error) => {
          clearComponent();
          notifyApp(createErrorNotification(`Import all dashboard failed - ${err.message}`));
        })
        .finally(() => {
          importOperations.clearAllDashboard();
          setLoading(false);
  ...
  return (
    <div className={s.flexJustify}>
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <Label className={s.disclaimerLabel}>{'set required parameters for the selected dashboards'}</Label>
      </div>
        ...
        {!importAllDone ? (
          ...
          <>
            ...
            <Label style={{ fontSize: '16px', marginTop: '10px' }}>
              {allSuccess ? 'All dashboards are successfully imported' : 'Failed to import the following dashboards'}
            </Label>
