...
import {DataSourceInstanceSettings, OptimizeVariableModel, SelectableValue} from "../../../../../packages/grafana-data";
...
export const DEFAULT_PAGINATION = { page: 0, size: 15 };
...
class OptimizeVariablePickerUnconnected extends PureComponent<Props, DomainFilterState> {
  ...
  getSelectedDomains = (appids: string | string[] | undefined) => {
    return this.getDomainsAPI(appids)
      .pipe(
    ...
    this.searchDomains(searchString, parentId, pagination)
      .pipe(
        ...
        tap((options: SelectableValue[]) => {
          ...
          this.setState((state) => ({domainsFilter: {...state.domainsFilter, loading: false, options: options, breadcrumbs: currBreadCrumb}}));
        }),
        take(1)
      )
      .subscribe();
  ...
  searchDomains(query: string, parentId: number, pagination?: Pagination): Observable<DatamartDomain[]> {
    ...
    return this.post(url, body)
      .pipe(
        tap((response) => this.setState((state) => ({domainsFilter: {...state.domainsFilter, count: response?.data?.count || 0 }}))),
  ...
  getDomainChildren(parentId: number, pagination?: Pagination): Observable<DatamartDomain[]> {
    const url = OptimizeVariablePickerUnconnected.API_CATALOGPROXY_DOMAINS_CHILDREN.replace('<parentId>', `${parentId}`);
    const body = {
      pagination: {
        page: pagination?.page || DEFAULT_PAGINATION.page,
        size: pagination?.size || DEFAULT_PAGINATION.size
  ...
  onDomainBreadCrumbsItemClick = (domain: SelectableValue) => {
    ...
    const breadcrumbs = domain.value === ROOT_RESOURCE_ID ? [] : this.state.domainsFilter.breadcrumbs.slice(0, index + 1);
    this.setState((state) => ({
      domainsFilter: {...state.domainsFilter, rootItem: domain.value, breadcrumbs: breadcrumbs},
    }));
  ...
  render(){
    ...
    return (
      ...
      <DomainFilter
        ...
        onSearch={(searchString: string) => this.loadDomains(searchString, domainsFilter.rootItem)}
