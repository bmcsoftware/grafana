        ...
        <CollapsableSection label={'Mail Settings'} isOpen={true}>
          ...
          <Field label="Subject">
            <JoditEditor
              ...
              value={sanitize(getValues().subject)}
              onChange={(value: any) => {
          ...
          <Field label="Custom message">
            <JoditEditor
              ...
              value={sanitize(getValues().message)}
              onChange={(value: any) => {
                  ...
                  {Object.keys(queryString.parse(getValues().filter.value) as any).map((key, i) => {
                    ...
                    return values === '' ? (
                      ...
                      <code className={styles.pins} key={key + i}>
                        <b>{key.replace('var-', '').toLocaleUpperCase()}</b> : {tryParseVariables(values)}
                      </code>
                    );
                  })}
                </div>
