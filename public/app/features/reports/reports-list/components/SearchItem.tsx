...
import { HistoryModal } from '../../report-console/components/lib/HistoryModal';
...
export const SearchItem: FC<Props> = ({ item, editable, onToggleChecked, style }) => {
  ...
  return (
    ...
    >
      ...
      <Tooltip content={'View last 10 executed schedules'}>
        <Icon
          name="history"
          size="lg"
          style={{ paddingRight: '8px' }}
          className={getInfoIconStyle(item.info.status, theme)}
          onClick={(_) => onShowLastSchedules(item.id, item.title)}
        />
      </Tooltip>
