import { PeriodType, Unit, UNITS, LeadingZero, ClockFormat, SchedulerObject } from './types';
import { dedup, range, sort } from './utils';
import cronstrue from 'cronstrue';

/**
 * Get human readable string from cron (using: https://www.npmjs.com/package/cronstrue)
 */
export function getReadableStringFromCron(cronString: string) {
  return cronstrue.toString(cronString);
}

/**
 *
 * Cron converter component functionality is based on:
 *    npm: https://www.npmjs.com/package/react-js-cron
 *    github: https://github.com/xrutayisire/react-js-cron
 *    demo: https://xrutayisire.github.io/react-js-cron/?path=/docs/reactjs-cron--demo
 */

/**
 * Get values from cron string
 */
export function getValuesFromCronString(cronString: string): SchedulerObject {
 ...
 */
export function getCronStringFromValues(scheduler: SchedulerObject, humanizeValue?: boolean) {
  const period = scheduler.period;
  const newMonths = period === 'year' && scheduler.months ? scheduler.months : [];
  const newMonthDays = (period === 'year' || period === 'month') && scheduler.monthDays ? scheduler.monthDays : [];
  const newWeekDays =
    (period === 'year' || period === 'month' || period === 'week') && scheduler.weekDays ? scheduler.weekDays : [];
  const newHours = period !== 'minute' && period !== 'hour' && scheduler.hours ? scheduler.hours : [];
  const newMinutes = period !== 'minute' && scheduler.minutes ? scheduler.minutes : [];
...
function parseStep(step: string, unit: Unit) {
  ...
  // return undefined to resolve eslint issue
  return undefined;
}
