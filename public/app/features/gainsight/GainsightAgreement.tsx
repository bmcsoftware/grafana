...
import { Preference, setImsUserPreferences } from '../../core/services/ims_srv';
...
class GainsightAgreement extends Component<any, any> {
  ...
  static getDerivedStateFromProps(props: any, state: any) {
    ...
    if (!_isEqual(props.preferences, state.preferences) && props.preferences instanceof Array) {
      return {
  ...
  handleEventListeners() {
    this.adaptAgreementRef.addEventListener &&
      this.adaptAgreementRef.addEventListener('onStateChange', (e: any) => {
        console.log('on submit state', e.detail);
  ...
  render() {
    ...
    if (this.state.userType === 'TENANT') {
      if (!this.state.preferences?.length || !this.props.isModal) {
        showAgreement = true;
      }
    } else if (this.state.userType === 'USER') {
      let tenantConsent;
      tenantConsent = this.state.preferences?.find(
        (pref: any) => pref.key === 'GS_DATA_COLLECTION_FOR_QUALITY_CONSENT' && pref.level === 'TENANT'
      );
      if (tenantConsent?.value === 'true') {
        if (this.props.isModal && this.state.preferences?.length === 2) {
          showAgreement = true;
        } else if (!this.props.isModal) {
          showAgreement = tenantConsent && JSON.parse(tenantConsent.value);
        }
      }
    }
    return (
      <div className="css-xs0vux">
        {showAgreement && !this.state.initialLoading && (
          <adapt-agreement
            ref={(elem: any) => {
