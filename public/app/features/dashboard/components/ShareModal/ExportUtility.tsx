...
const ExportUtility: React.FC<Props> = (props) => {
  ...
  const isTablePanel =
    props.panel?.type === 'table' || props.panel?.type === 'table-old' || props.panel?.type === 'bmc-ade-cross-tab' || props.panel?.type === 'bmc-table-panel';
      ...
      <div className="share-modal-header">
        ...
        <div className="share-modal-content">
          ...
          {selectedReportType?.value === ReportType.PDF && showTableScaling ? (
            <div className="gf-form">
              <label className="gf-form-label width-8" htmlFor="pdf-table-scaling">
...
const getDashReportTypes = (dashboard: any, panel: any): any => {
  ...
  const canCSV = dashboard.panels.some((panel: any) => panel.type === 'table' || panel.type === 'table-old'  || panel.type === 'bmc-table-panel');
  if (!canCSV) {
...
const showTableScalingOption = (props: any, state: any): boolean => {
  ...
  if (props.panel && (props.panel.type === 'table' || props.panel.type === 'table-old' || props.panel.type === 'bmc-table-panel')) {
    showScaling = true;
  } else {
    showScaling =
      props.dashboard.panels.some((panel: any) => panel.type === 'table' || panel.type === 'table-old' || panel.type === 'bmc-table-panel') &&
      state.selectedLayout.value === PDFLayout.SIMPLE;
  }
  return showScaling && getGrafanaFeatureStatus(FEATURE_CONST.EXPORT_COMPLETE_TABLE);
};
