            ...
            <div className="gf-form">
              ...
              <div className="width-16">
                <RadioButtonGroup
                  ...
                  value={isFixedLayout}
                  onChange={() => { setState({ ...state, isFixedLayout: !state.isFixedLayout }); }}
                  fullWidth
                />
              </div>
            </div>            
...
const showTableScalingOption = (props: any, state: any): boolean => {
  ...
  if (props.panel && (props.panel.type === 'table' || props.panel.type === 'table-old')) {
    showScaling = true;
  } else {
    showScaling = props.dashboard.panels.some((panel: any) => panel.type === 'table' || panel.type === 'table-old')
