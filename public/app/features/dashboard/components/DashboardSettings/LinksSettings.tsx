...
export function LinksSettings({ dashboard, sectionNav, editIndex }: SettingsPageProps) {
  ...
  if (isEditing) {
    const title = isNew ? `${t('bmcgrafana.dashboards.settings.links..title-new-link','New link')}` : `${t('bmcgrafana.dashboards.settings.links.title-edit-link','Edit link')}`;
    const description = isNew ? `${t('cdescription-new-link','Create a new link on your dashboard')}` : `${t('bmcgrafana.dashboards.settings.links.description-edit-link','Edit a specific link of your dashboard')}`;
    pageNav = {
      text: title,
