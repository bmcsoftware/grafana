...
export const useModifyFields = (action: string, uid: string | undefined) => {
  ...
  const validateRawQuery = useCallback(() => {
    ...
    calcFieldsSrv
      ...
      .then((resp: any) => {
        appEvents.emit(AppEvents.alertSuccess, ['Query Validated Successfully']);
        dispatch({ type: UPDATE_FIELDS, payload: { rawQueryValidated: true } });
      })
      .catch((e: any) => {
        let customErrMsg = 'Unknown failure';
        if (e.data?.[0]) {
          customErrMsg = `${e.data[0].messageNumber ?? '500'}: ${e.data[0].messageText ?? 'Failure'}, ${
            e.data[0].messageAppendedText ?? ''
          }`;
        }
        appEvents.emit(AppEvents.alertError, ['Query not validated', customErrMsg]);
