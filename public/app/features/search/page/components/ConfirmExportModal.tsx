...
export const ConfirmExportModal: FC<Props> = ({ onExportDone, results, isOpen, onDismiss }) => {
  ...
  let text = t('bmc.search.comfirm-export', 'Do you want to export the {{dashCount}} selected dashboard(s)?', {dashCount});
  ...
  return isOpen ? ( !isExportDone ? (
    ...
    <Modal className={styles.modalExportStatus} title={t('bmc.search.export-status', 'Export Status')} isOpen={isExportDone} onDismiss={onExportDone}>
       <>
          <div className={styles.contentExportStatus}>Export Successfull: <span className={styles.exportSuccess}>{dashCount-failedExport.length}</span></div>
          <div className={styles.contentExportStatus}>Export Failed: <span className={styles.exportFailed}>{failedExport.length}</span></div>
          {
            (failedExport.length === 0) ? (
              null
            ) : (
