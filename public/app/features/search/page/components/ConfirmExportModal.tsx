...
export const ConfirmExportModal: FC<Props> = ({ onExportDone, results, isOpen, onDismiss }) => {
  ...
  const exportDashboards = () => {
    ...
    _exportDashboards({
      ...
      .then(() => {
        onExportDone();
        notifyApp.success(t('bmc.search.export-success', 'Exported successfully'));
      })
      .catch((err: any) => {
        notifyApp.error(t('bmc.search.export-failed', 'Export failed'), err?.data?.message || err.message);
      })
      .finally(() => {
