...
const matchTooltipContent = () => {
  return t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.match-any-desc', 'Enabling this returns annotations that match any of the tags specified below');
};

/*BMC Change: To enable localization for below text*/
const tagsTooltipContent = () => {
  return (
    <Trans i18nKey={'bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.tag-field-desc'}><div>Specify a list of tags to match. To specify a key and value tag use `key:value` syntax.</div></Trans>
  )
};
...
const annotationTypes = (): Array<SelectableValue<GrafanaAnnotationType>> => {
  return ([
    {
      label: t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.filter-by.dashboard-label', 'Dashboard'),
      value: GrafanaAnnotationType.Dashboard,
      description: t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.filter-by.dashboard-desc', 'Query for events created on this dashboard and show them in the panels where they where created'),
    },
    {
      ...
      description: t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.filter-by.tags-desc', 'This will fetch any annotation events that match the tags filter'),
    },
  ])
};
...
const queryTypes = (): Array<SelectableValue<GrafanaQueryType>> => {
  return ([
    {
      label: t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.query-type.annotation-label', 'Annotations & Alerts'),
      value: GrafanaQueryType.Annotations,
      description: t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.query-type.annotation-desc', 'Show annotations or alerts managed by grafana'),
    },
    {
      label: t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.query-type.time-region-label', 'Time regions'),
      value: GrafanaQueryType.TimeRegions,
      description: t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.query-type.time-region-desc', 'Configure a repeating time region'),
    },
  ])
};
...
export default function AnnotationQueryEditor({ query, onChange }: Props) {
  ...
  return (
    <FieldSet className={styles.container}>
      {/*BMC Change: To enable localization for below text*/}
      <Field label={t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.query-type-label', "Query type")}>
        <Select
      ...
      {grafanaQueryType === GrafanaQueryType.Annotations && (
        <>
          {/*BMC Change: To enable localization for below text*/}
          <Field label={t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.filter-by-label', "Filter by")}>
            <Select
          ...
          {/*BMC Change: To enable localization for below text*/}
          <Field label={t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.max-limit-label', "Max limit")}>
            <Select
          ...
          {type === GrafanaAnnotationType.Tags && (
            <>
              <Field label={t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.match-any-label', "Match any")} description={matchTooltipContentMemo}>
                <Switch id="grafana-annotations__match-any" value={matchAny} onChange={onMatchAnyChange} />
              </Field>
              <Field label={t('bmcgrafana.dashboards.settings.annotation.edit-form.query-editor.filter-by.tags-label', "Tags")} description={tagsTooltipContentMemo}>
                <TagFilter
                  allowCustomValue
